document.addEventListener('DOMContentLoaded', () => {
    ...
    const desktopYoutubeUrl = document.getElementById('desktopYoutubeUrl');
    const desktopTopicInput = document.getElementById('desktopTopicInput');
    const desktopPromptInput = document.getElementById('desktopPromptInput');
    const desktopGenerateBtn = document.getElementById('desktopGenerateBtn');
    ...
    // Desktop Content Panel
    desktopGenerateBtn?.addEventListener('click', handleDesktopGenerate);
    ...
    // --- DESKTOP GENERATION ---
    async function handleDesktopGenerate() {
        if (!accessToken) {
            addChatMessage('ai', 'Google 로그인이 필요합니다.');
            return;
        }

        const youtubeUrl = desktopYoutubeUrl?.value.trim();
        const topicInput = desktopTopicInput?.value.trim();
        const promptInput = desktopPromptInput?.value.trim();

        if (!youtubeUrl && !topicInput && !promptInput) {
            addChatMessage('ai', 'YouTube URL, 주제 또는 프롬프트 중 하나는 입력해주세요.');
            return;
        }
        ...
        // Add AI prompt suffix if provided
        if (promptInput && contentType !== 'prompt') {
            content += `\n\n추가 지시사항: ${promptInput}`;
        }

        // Start generation
        desktopGenerateBtn.disabled = true;
        desktopGenerateBtn.textContent = '생성 중...';
        
        try {
            await handleContentGeneration(content);
        } finally {
            desktopGenerateBtn.disabled = false;
            desktopGenerateBtn.textContent = '🚀 포스트 생성';
            
            // Clear inputs after successful generation
            if (desktopYoutubeUrl) desktopYoutubeUrl.value = '';
            if (desktopTopicInput) desktopTopicInput.value = '';
            if (desktopPromptInput) desktopPromptInput.value = '';
        }
    }
